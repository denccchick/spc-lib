...
class BaseControlChart:
    def __init__(self, data, datetimes=None, target=None, usl=None, lsl=None):
        ...
        self.ucl_disp = None
        self.lcl_disp = None
        self.main_label = "Main statistic"
        self.disp_label = "Dispersion statistic"
        self.sigma_est = None
    ...
    def plot_rules(self, start=None, end=None, last_n=30, show_spec=False):
        from spc_lib.visualization import plot_rules_violations
        return plot_rules_violations(self, start, end, last_n, show_spec)
    ...
    def capability(self, usl=None, lsl=None):
        ...
        if self.stat_main is None or self.cl_main is None:
            raise ValueError("Chart has not been fitted (call .fit() first)")

        # Process center
        mu = self.cl_main

        # Sigma estimation (within-subgroup)
        if self.sigma_est is not None:
            ...
            # Fallback (total variance, gives Pp instead of Cp)
        ...
        # Cpk is the minimum of available one-sided indices
        if usl is not None and lsl is not None:
            cpk = min(cpu, cpl)
